...
// Server-side deterministic noise for Headless Terrain Collision
class SimpleNoise {
...
io.on('connection', (socket) => {
    ...
    socket.on('createGame', (playerName) => {
        ...
        sessions[roomId] = { 
            ...
            players: {}, blocks: {}, drops: {}, mobs: {}, startTime: Date.now()
        };
    ...
    function joinRoom(socket, roomId, playerName) {
        ...
        socket.emit('world_snapshot', {
            seed: room.seed, players: room.players, blocks: room.blocks, drops: room.drops, mobs: room.mobs,
    ...
    socket.on('requestPickup', (dropId) => {
        ...
        if(room && room.drops[dropId]) {
            ...
            delete room.drops[dropId]; // ✨ ATOMIC SERVER OPERATION: Kills 21x torch duplication
    ...
    socket.on('requestMobAttack', (data) => {
        ...
        if (room && room.mobs[data.id]) {
            ...
            if (room.mobs[data.id].health <= 0) { delete room.mobs[data.id]; io.in(socket.roomId).emit('mobKilled', data.id); }
        }
    });

    socket.on('playerRespawn', () => {
        if(socket.roomId && sessions[socket.roomId] && sessions[socket.roomId].players[socket.id]) {
            sessions[socket.roomId].players[socket.id].health = 100;
        }
    });
    ...
    socket.on('disconnect', () => {
        if(socket.roomId && sessions[socket.roomId]) {
            ...
            if(Object.keys(sessions[socket.roomId].players).length === 0) delete sessions[socket.roomId]; 
            broadcastLobby(); 
        }
    });
});

// ✨ 20 TPS SERVER GAME LOOP: Drives Mobs and Combat
setInterval(() => {
    for (let roomId in sessions) {
        const room = sessions[roomId]; const noise = new SimpleNoise(room.seed); const playerIds = Object.keys(room.players);
        ...
        if (Object.keys(room.mobs).length < 10 && Math.random() < 0.05) {
            ...
            let elevation = (noise.getNoise(mx * 0.015, mz * 0.015) + 1) * 8; let my = Math.floor(elevation + (new SimpleNoise(1337).getNoise(mx * 0.06, mz * 0.06)) * 3) + 2;

            const id = 'mob_' + globalIdCounter++; const isZombie = Math.random() > 0.5;
            room.mobs[id] = { id: id, type: isZombie ? 'zombie' : 'archer', weapon: isZombie ? 'wooden_sword' : 'bow', x: mx, y: my + 1.5, z: mz, ry: 0, rx: 0, health: 100, isMoving: false, isAttacking: false, attackTimer: 0 };
        }

        // 2. Headless Mob AI & Combat Validation
        for (let mobId in room.mobs) {
            ...
            if (mob.attackTimer > 0) mob.attackTimer -= 0.05; // 50ms tick depletion

            if (closestPlayer && minD < 40) {
                const angle = Math.atan2(closestPlayer.x - mob.x, closestPlayer.z - mob.z); mob.ry = angle;
                if (mob.type === 'zombie') {
                    if (minD > 1.8) {
                        mob.x += Math.sin(angle) * 0.15; mob.z += Math.cos(angle) * 0.15; mob.isMoving = true; mob.isAttacking = false;
                    } else {
                        mob.isMoving = false;
                        if (mob.attackTimer <= 0) {
                            mob.attackTimer = 1.5; mob.isAttacking = true;
                            room.players[closestPlayer.id].health -= 15;
                ...
                let elevation = (noise.getNoise(mob.x * 0.015, mob.z * 0.015) + 1) * 8; mob.y = Math.floor(elevation + (new SimpleNoise(1337).getNoise(mob.x * 0.06, mob.z * 0.06)) * 3) + 3.5;
